      ...
      <div className="w-full h-full flex justify-center items-center">
        <form className="w-[300px] sm:w-[400px] h-auto flex flex-col justify-center items-center rounded-3xl shadow-3xl p-6" style={{background: "rgba(255,255,255,0.80)"}} onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex flex-row justify-start items-center w-full h-auto gap-x-2">
            <div className="flex flex-col justify-start items-start w-1/2 h-auto overflow-hidden gap-y-4 sm:gap-y-1">
              <label htmlFor="FirstName" className="font-primary text-secondary text-sm sm:text-lg h-3 sm:h-6">{t("Firstname")}</label>
              ...
              <div className="w-full h-6">
                {errors?.firstName && 
                  <motion.p 
                    ...
                    className="h-6 text-xs text-primary font-tertiary">{t(errors.firstName.message ? errors.firstName.message : "Firstname is required.")}
                  </motion.p>
                }
              </div>
            </div>
            <div className="flex flex-col justify-start items-start w-1/2 h-auto overflow-hidden gap-y-4 sm:gap-y-1">
              <label htmlFor="FirstName" className="font-primary text-secondary text-sm sm:text-lg h-3 sm:h-6">{t("Lastname")}</label>
              ...
              <div className="w-full h-6">
                {errors?.lastName && 
                  <motion.p 
                    ...
                    exit="hidden"
                    variants={fadeIn("up","", 0, 1)}
                    className="h-6 text-xs text-primary font-tertiary">{t(errors.lastName.message ? errors.lastName.message : "Lastname is required.")}
                  </motion.p>
                }
              </div>
            </div>
            ...
            <div className="w-full h-10">
              {errors?.password && 
                <motion.p 
                  ...
                  exit="hidden"
                  variants={fadeIn("up","", 0, 1)}
                  className="h-10 text-xs text-primary font-tertiary">{t(errors.password.message ? errors.password.message : "Confirmation Password is required.")}
                </motion.p>
              }
            </div>
          </div>
